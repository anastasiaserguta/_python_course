...
print('Четные числа:', *{num for num in all_nums if '.' not in num and int(num) % 2 == 0})
print('Нечетные числа:', *{num for num in all_nums if '.' not in num and int(num) % 2 != 0})
print('Числа с плавающей точкой:', *[num for num in all_nums if '.' in num])
print('Сумма всех чисел кратных 5:', sum([int(num) for num in all_nums if '.' not in num and int(num) % 5 == 0]))
