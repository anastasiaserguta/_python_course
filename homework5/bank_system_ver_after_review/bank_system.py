...
class Currency(
    enum.Enum
):  # Класс, содержащий коды и наименования валют, а также примерные значения курсов для конвертации.
    BYN = "933"
    USD = "840"
    EUR = "978"
    RUB = "643"
...
class Bank:  # Класс БАНК. Инициирует базу данных клиентов, сериализует/десериализует ее, содержит метод "обратной связи" с клиентом.
    ...
    def serialization(client_base):
        cwd = getcwd()
        base_dir = path.join(
            cwd, "homework5", "bank_system_ver_after_review", "client_base.json"
        )
...
class BankAccount:  # Класс БАНКОВСКИЙ СЧЕТ. Основной класс для работы со счетом (пополнение, снятие, просмотр баланса и всех транзакций, включая методы валидации обрабатываемых сумм.)
    ...
    def check_any_open_accounts(
        self, code_currency="933"
    ):  # Проверка наличия открытых аккаунтов.
        try:
    ...
    def replenish_balance(self, code_currency, amount):  # Пополнение баланса.
        ...
        self.balance = BankAccount.convert_to_decimal(
            self.client_base[self.login][1][code_currency]
        )
        self.client_base[self.login][1][code_currency] = self.balance + self.amount
        self.status = "SUCCES"
        print(
            f"Your balance in {Currency(code_currency).name} is {self.client_base[self.login][1][code_currency]}."
        )
    ...
    def withdraw_money(self, code_currency, amount):  # Снятие со счета.
        ...
        if BankAccount.is_valid_operation(self.balance, self.amount):
            ...
            print(
                f"Your amount is equal 0 or balance in {Currency(code_currency).name} is not open or equal 0 or less than amount required for the transfer after conversion."
            )
            self.status = "DENIED"
            return False
    ...
    def transfer_money():  # Метод, определяющий необходимость использования одного существующего счета для пополнения другого.
        answer = input(
            'Do you want transfer money from an existing account? Enter "yes" or "no": '
        ).lower()
        if answer == "yes":
            return True
        elif answer == "no":
    ...
    def record_all_operations(
        self, action_name, code_currency, amount, code_currency_from=0
    ):  # Запись всех транзакций, включая их статус : УСПЕШНО/ОТКАЗАНО.
        cwd = getcwd()
        directory_folder = path.join(
            cwd,
            "homework5",
            "bank_system_ver_after_review",
            "client_operations",
            str(self.login) + ".txt",
        )
...
class BankClient:  # Класс БАНКОВСКИЙ КЛИЕНТ. Инициирует клиентов, проверяет их на предмет наличия в базе.
    ...
    def check_new_client(self):
        if (
            self.name not in self.client_base.keys()
            and self.name != "#3"
            and self.name != "help"
        ):
    ...
    def create_client_account(self):
        ...
        return self.client_base[self.name].append(
            {
                Currency.BYN.value: "not open",
                Currency.USD.value: "not open",
                Currency.EUR.value: "not open",
                Currency.RUB.value: "not open",
            }
        )
    ...
    try:
        ...
        if action == 1:
            ...
            if client.check_new_client():
                ...
                print(
                    f'Account with this login "{client_name}" exist or you use system command like [help, #3], please, try again:('
                )
        elif action == 2:
            ...
            try:
                for _ in range(5):
                    if active_client.check_active_client():
                        ...
                        while command != "stop":
                            ...
                            print(
                                *[
                                    f"Code of {i.name} is {i.value}."
                                    for i in list(Currency)
                                    if "rate" not in i.name
                                ],
                                sep="\n",
                            )  # какая-то имба получилась
                            code_currency = str(
                                input("Select the currency (enter only code): ")
                            )
                            try:
                                print(
                                    f"The account in {Currency(code_currency).name} is selected."
                                )
                            ...
                            if active_client.check_any_open_accounts(code_currency):
                                ...
                                if command == "+":
                                    if not BankAccount.transfer_money():
                                        try:
                                            amount_for_replenish = input(
                                                "Enter amount: "
                                            )
                                            active_client.replenish_balance(
                                                code_currency, amount_for_replenish
                                            )
                                            active_client.record_all_operations(
                                                "withdrawal from account",
                                                code_currency,
                                                amount_for_replenish,
                                            )
                                        except ValueError:
                                            print("For amount use only digits!")
                                            continue
                                    else:
                                        code_currency_from = str(
                                            input(
                                                f"Select the currency code from which you want to transfer the amount (except {Currency(code_currency).name}): "
                                            )
                                        )
                                        ...
                                        try:
                                            print(
                                                f"The account in {Currency(code_currency_from).name} is selected for the transfer."
                                            )
                                        except ValueError as value:
                                            print(f"{value}!")
                                            continue
                                        try:
                                            if code_currency == Currency.BYN.value:
                                                amount_for_replenish = BankAccount.convert_to_decimal(
                                                    input(
                                                        f"Enter amount in {Currency.BYN.name}: "
                                                    )
                                                )
                                                if (
                                                    code_currency_from
                                                    ...
                                                    amount_for_withdraw = (
                                                        amount_for_replenish
                                                        / Currency.BYN_USD_rate.value
                                                    )
                                                    if active_client.withdraw_money(
                                                        code_currency_from,
                                                        amount_for_withdraw,
                                                    ):
                                                        active_client.replenish_balance(
                                                ...
                                                amount_for_replenish = BankAccount.convert_to_decimal(
                                                    input(
                                                        f"Enter amount in {Currency.USD.name}: "
                                                    )
                                                )
                                                if (
                                                    code_currency_from
                                                ...
                                                amount_for_replenish = BankAccount.convert_to_decimal(
                                                    input(
                                                        f"Enter amount in {Currency.EUR.name}: "
                                                    )
                                                )
                                                if (
                                                    code_currency_from
                                                ...
                                                amount_for_replenish = BankAccount.convert_to_decimal(
                                                    input(
                                                        f"Enter amount in {Currency.RUB.name}: "
                                                    )
                                                )
                                                if (
                                                    code_currency_from
                                    ...
                                    amount_for_withdraw = (
                                        BankAccount.convert_to_decimal(
                                            input("Enter amount: ")
                                        )
                                    )
                                    try:
                                        active_client.withdraw_money(
                                            code_currency, amount_for_withdraw
                                        )
                                        active_client.record_all_operations(
                                            "withdrawal from account",
                                            code_currency,
                                ...
                                print(
                                    f"ATTENTION! Account in {Currency(code_currency).name} is not open."
                                )
                                if BankAccount.open_account():
                                    active_client.create_account(code_currency)
                    else:
                        print(
                            f"Uncorrect ID. Try again ({attempts_enter} attempts) or use command '#3' ('help')!"
                        )
                        attempts_enter -= 1
                        id = str(input("Enter your ID (9 numbers): "))
                        if id == "#3" or id == "help":
                ...
                print(
                    f"Account with login {login} doesn't exist. Try again or create new account!"
                )
        elif action == 3:
            print("OK.")
            break
