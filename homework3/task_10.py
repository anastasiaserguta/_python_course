...
def start_game():    # Функция старта, где пользователь определяет некоторые условия игры.
    ...
    def is_valid(digit_user): # Функция проверки корректности введенного пользователем числа.
        flag = False
        while flag != True:
        ...
        if flag == True: # При вводве корректного значения запускаем основной блок игры.
            random_game(digit_user)
            
            

    def random_game(digit_user):   # Основая часть игры.                              
        total = 1
